...
import prefs, {getPref, setPref} from "./prefs.js";
import storage, {
  CURRENT_FORMAT, formatKey, saltKey, hmacSecretKey, setKeyCallback,
  setHmacSecretCallback, setUseridCallback
} from "./storage.js";
import {deriveKey, generateRandom, getDigest, importHmacSecret} from "./crypto.js";
import {
...
export async function getState()
{
  ...
  if (await getPref("usersalt", null))
    return "set";
  else
    return "unset";
}
...
async function setUserid(masterPassword)
{
  let usersalt = await getPref("usersalt", null);
  if (!usersalt)
  {
    usersalt = generateRandom(16);
    setPref("usersalt", usersalt);
  }
  usersalt = await importHmacSecret(usersalt);
  userid = await getDigest(usersalt, masterPassword);
}

export async function changePassword(masterPassword, noLock)
{
  await setUserid(masterPassword);
