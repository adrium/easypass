...
export async function getPassword(passwordData)
{
  ...
  if (passwordData.type.startsWith("generated"))
  {
    let {type, site, name, revision, length, lower, upper, number, symbol} = passwordData;
    let params = {
      masterPassword: masterPasswordCallback(),
      domain: site,
      type, name, revision, length, lower, upper, number, symbol
    };
...
export const addGenerated = locked(async function({type, site, name, revision, length, lower, upper, number, symbol, notes}, replaceExisting)
{
